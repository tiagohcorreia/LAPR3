...
BEGIN
    IF INSERTING THEN
        ...
        VALUES (log_seq.NEXTVAL, :NEW.id, CURRENT_TIMESTAMP, 'Criação', 'Data: ' || TO_CHAR(:NEW.data, 'YYYY-MM-DD') || ', Validade: ' || :NEW.validade);
    ELSIF UPDATING THEN
        INSERT INTO Log_Operacao (log_id, operacao_id, instante, tipo_operacao, dados_operacao)
        VALUES (log_seq.NEXTVAL, :NEW.id, CURRENT_TIMESTAMP, 'Alteração', 'Data: ' || TO_CHAR(:NEW.data, 'YYYY-MM-DD') || ', Validade: ' || :NEW.validade);
    END IF;
END;
/
